...
-- Trigger/Funcion que añade automaticamente id_usuarioj y url_tienda a la tabla usuario
	
	
	CREATE OR REPLACE FUNCTION asignar_datos_por_defecto_usuario()
	RETURNS TRIGGER AS $$
	BEGIN
	    
	    IF NEW.id_usuarioJ IS NULL THEN
	        NEW.id_usuarioJ := 0;
	    END IF;
	
	    
	    IF NEW.url_tienda IS NULL THEN
	        NEW.url_tienda := 'monono.store';
	    END IF;
	...
	CREATE TRIGGER trigger_asignar_datos_por_defecto
	BEFORE INSERT ON Usuario
	FOR EACH ROW
	EXECUTE FUNCTION asignar_datos_por_defecto_usuario();

-- Trigger para asignar automaticamente la tienda a un juego
	CREATE OR REPLACE FUNCTION asignar_url_tienda_default_videojuego()
	RETURNS TRIGGER AS $$
	BEGIN
	    IF NEW.url_tienda IS NULL THEN
	        NEW.url_tienda := 'monono.store';
	...
	CREATE TRIGGER trigger_url_tienda_default
	BEFORE INSERT ON Videojuego
	FOR EACH ROW
	EXECUTE FUNCTION asignar_url_tienda_default_videojuego();


-- Trigger para crear automaticamente Lista y carro de usuario al insertarlo
	CREATE OR REPLACE FUNCTION crear_lista_y_carro_usuario()
	RETURNS TRIGGER AS $$
	BEGIN
	    -- Verificar si ya existe la lista
	    IF NOT EXISTS (
	        SELECT 1 FROM ListaDeseados WHERE id_lista = NEW.id_usuario
	    ) THEN
	        INSERT INTO ListaDeseados(id_lista, id_usuario, id_ranking)
	        VALUES (NEW.id_usuario, NEW.id_usuario, 1);
	    END IF;
	
	    -- Verificar si ya existe el carro
	    IF NOT EXISTS (
	        SELECT 1 FROM CarroCompras WHERE id_carro = NEW.id_usuario
	    ) THEN
	    ...
	    UPDATE Usuario
	    SET id_lista = NEW.id_usuario,
	        id_carro = NEW.id_usuario
	...
	CREATE TRIGGER trigger_crear_lista_y_carro
	AFTER INSERT ON Usuario
	FOR EACH ROW
	EXECUTE FUNCTION crear_lista_y_carro_usuario();
...
-- TC08 Actualizar juego si se es jefe
	...
	BEGIN
	    ...
	    IF v_rol_usuario IS DISTINCT FROM 'Jefe de tienda' THEN
	        RAISE EXCEPTION 'Permiso denegado: solo un Jefe de tienda puede actualizar videojuegos.';
	    END IF;
...
-- TC09 Usuario añade un juego a su lista
	...
	$$ LANGUAGE plpgsql;
			...
			-- Funcion para eliminar un juego de la lista

			
	CREATE OR REPLACE FUNCTION eliminar_juego_de_lista_deseos(
	    p_username VARCHAR,
	    p_game_name VARCHAR
	)
	RETURNS VOID AS $$
	DECLARE
	    ...
	    v_id_lista INT;
	BEGIN
	    -- Recolección del ID del usuario
	    SELECT id_usuario INTO v_usuario_id
	    FROM Usuario
	    WHERE username = p_username;
	    ...
	    -- Recolección del ID del videojuego
	    SELECT game_id INTO v_game_id
	    FROM Videojuego
	    WHERE game_name = p_game_name;
	    ...
	    -- Recolección del ID de la lista de deseos
	    SELECT id_lista INTO v_id_lista
	    FROM ListaDeseados
	    WHERE id_usuario = v_usuario_id;
	    ...
	    -- Eliminar el videojuego de la lista de deseos
	    DELETE FROM VideojuegoxLista
	    WHERE game_id = v_game_id AND id_lista = v_id_lista;
	
	    RAISE NOTICE 'Juego "%" eliminado de la lista de deseos del usuario "%".', p_game_name, p_username;
	END;
	$$ LANGUAGE plpgsql;
